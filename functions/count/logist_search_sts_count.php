<?php
function logist_search_sts_count()
{
    global $conn;
    if ($_SESSION['user_status']==2 or $_SESSION['user_status']==4) { ?>
        ...
        if($_SESSION['user_name']=='Бағлан'){
            ...
            $query = $conn->query("SELECT count(client_price) as total3 FROM clients WHERE client_status = '2,1.Алматы' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Алматы (<?= $row['total3'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total2 FROM clients WHERE client_status = '2.Подтвержден' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Подтвержден (<?= $row['total2'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total6 FROM clients WHERE client_status = '4,1.Ожидает доставки' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Ожидает доставки (<?= $row['total6'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total8 FROM clients WHERE client_status = '5.В Пути' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">В Пути (<?= $row['total8'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '4,3.Поступил' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Поступил (<?= $row['total9'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '6.Вручено' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Вручено (<?= $row['total9'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '2,2.Вручено АЛ' AND client_confirm = 'Бағлан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Вручено Ал (<?= $row['total9'] ?>)</th>
            <?php
        ...
        elseif($_SESSION['user_name']=='Асылжан'){
            ...
            <th class="text-center main-status">Поступил (<?= $row['total9'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '6.Вручено' AND client_confirm = 'Асылжан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Вручено (<?= $row['total9'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '2,2.Вручено АЛ' AND client_confirm = 'Асылжан'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Вручено Ал (<?= $row['total9'] ?>)</th>
            <?php
        ...
        <?php }
            ...
            <th class="text-center main-status">Поступил (<?= $row['total9'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '6.Вручено'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Вручено (<?= $row['total9'] ?>)</th>
            <?php
            $query = $conn->query("SELECT count(client_price) as total9 FROM clients WHERE client_status = '2,2.Вручено АЛ'");
            $row = mysqli_fetch_assoc($query); ?>
            <th class="text-center main-status">Вручено Ал (<?= $row['total9'] ?>)</th>
            <?php
